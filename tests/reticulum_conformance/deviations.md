...
| RCNF-010 | LXMF propagation client | The embedded runtime supports official propagation-node announce discovery, propagated message packing and stamping, identified Link packet/Resource upload with proof validation, and bounded `/get` synchronization. Propagation-node service mode remains a separate opt-in capability. |
