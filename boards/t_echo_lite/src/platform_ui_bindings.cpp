...
int readFilteredBatteryLevel(::boards::t_echo_lite::TEchoLiteBoard& board)
{
...
bool rtc_ready()
{
    return ::boards::t_echo_lite::TEchoLiteBoard::instance().isRTCReady();
}
...
{
    return ::boards::t_echo_lite::TEchoLiteBoard::instance().hasKeyboard();
}
...
{
    return ::boards::t_echo_lite::TEchoLiteBoard::instance().getBrightness();
}

void set_screen_brightness(uint8_t level)
{
    ::boards::t_echo_lite::TEchoLiteBoard::instance().setBrightness(level);
}
...
void trigger_haptic()
{
    ::boards::t_echo_lite::TEchoLiteBoard::instance().vibrator();
}

uint8_t default_message_tone_volume()
{
    return ::boards::t_echo_lite::TEchoLiteBoard::instance().getMessageToneVolume();
}

void set_message_tone_volume(uint8_t volume_percent)
{
    ::boards::t_echo_lite::TEchoLiteBoard::instance().setMessageToneVolume(volume_percent);
}

void play_message_tone()
{
    ::boards::t_echo_lite::TEchoLiteBoard::instance().playMessageTone();
}
...
bool gps_ready()
{
    return ::boards::t_echo_lite::TEchoLiteBoard::instance().isGPSReady();
}

bool gps_supported()
{
    return ::boards::t_echo_lite::TEchoLiteBoard::instance().hasGPSHardware();
}

int power_tier()
{
    return ::boards::t_echo_lite::TEchoLiteBoard::instance().getPowerTier();
}
...
void logGnssSnapshotUiFlow(bool has_snapshot, std::size_t sat_count, const GnssStatus& status)
{
    ...
    Serial.printf("[T-Echo Lite][gps][ui] snapshot has=%u count=%u status_view=%u status_use=%u hdop=%.1f fix=%u\n",
...
GpsState get_data()
{
    GpsState gps = ::boards::t_echo_lite::TEchoLiteBoard::instance().gpsData();
    if (gps.valid)
    {
        return gps;
    }
...
bool get_gnss_snapshot(GnssSatInfo* out, std::size_t max, std::size_t* out_count, GnssStatus* status)
{
    ...
    const bool has_snapshot = ::boards::t_echo_lite::TEchoLiteBoard::instance().gpsGnssSnapshot(
...
GpsDiagnosticsSnapshot diagnostics()
{
    ...
    snapshot.supported = ::boards::t_echo_lite::TEchoLiteBoard::instance().hasGPSHardware();
    snapshot.enabled = is_enabled();
    snapshot.powered = is_powered();
    snapshot.ready = ::boards::t_echo_lite::TEchoLiteBoard::instance().isGPSReady();
...
{
    return ::boards::t_echo_lite::TEchoLiteBoard::instance().gpsLastMotionMs();
}

bool is_enabled()
{
    return ::boards::t_echo_lite::TEchoLiteBoard::instance().gpsEnabled();
}

bool is_powered()
{
    return ::boards::t_echo_lite::TEchoLiteBoard::instance().gpsPowered();
}

void set_enabled(bool enabled)
{
    ::boards::t_echo_lite::TEchoLiteBoard::instance().setGpsEnabled(enabled);
}

void set_collection_interval(uint32_t interval_ms)
{
    ::boards::t_echo_lite::TEchoLiteBoard::instance().setGpsCollectionInterval(interval_ms);
}

void set_power_strategy(uint8_t strategy)
{
    ::boards::t_echo_lite::TEchoLiteBoard::instance().setGpsPowerStrategy(strategy);
}

void set_gnss_config(uint8_t mode, uint8_t sat_mask)
{
    ::boards::t_echo_lite::TEchoLiteBoard::instance().setGpsConfig(mode, sat_mask);
}

void set_external_nmea_config(uint8_t output_hz, uint8_t sentence_mask)
{
    ::boards::t_echo_lite::TEchoLiteBoard::instance().setGpsExternalNmeaConfig(output_hz, sentence_mask);
}

void set_motion_idle_timeout(uint32_t timeout_ms)
{
    ::boards::t_echo_lite::TEchoLiteBoard::instance().setGpsMotionIdleTimeout(timeout_ms);
}
...
void resume_runtime()
{
    ::boards::t_echo_lite::TEchoLiteBoard::instance().resumeGps();
}
