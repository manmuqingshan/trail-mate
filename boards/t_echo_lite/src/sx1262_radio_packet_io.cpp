...
SPIClass& radioSpi()
{
    const auto& spi = ::boards::t_echo_lite::kBoardProfile.lora.spi;
...
{
    const auto& profile = ::boards::t_echo_lite::kBoardProfile.lora;
    if (profile.rf_vc1 < 0 || profile.rf_vc2 < 0)
    {
        return;
    }
    pinMode(profile.rf_vc1, OUTPUT);
    pinMode(profile.rf_vc2, OUTPUT);
    digitalWrite(profile.rf_vc1, transmit ? HIGH : LOW);
    digitalWrite(profile.rf_vc2, transmit ? LOW : HIGH);
}
...
bool Sx1262RadioPacketIo::begin()
{
    ...
    initialized_ = true;
    const auto& profile = ::boards::t_echo_lite::kBoardProfile;
