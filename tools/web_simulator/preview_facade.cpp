...
{
  public:
    bool sendText(chat::ChannelId, const std::string&, chat::MessageId* id, chat::NodeId) override
    {
        if (id) *id = ++next_id;
        return true;
    }
    ...
    {
        chat::MeshCapabilities c;
        c.supports_unicast_text = true;
        c.supports_node_info = true;
        return c;
    }
...
class MemoryBlob final : public chat::IMeshPeerDirectoryBlobStore
{
  public:
    ...
    {
        out = data;
        return data.empty() ? chat::MeshPeerDirectoryBlobLoadResult::Missing : chat::MeshPeerDirectoryBlobLoadResult::Loaded;
    }
    bool saveBlob(const uint8_t* bytes, std::size_t size) override
    {
...
class PreviewApp final : public app::IAppFacade
{
  public:
    ...
    void getEffectiveUserInfo(char* a, std::size_t n, char* b, std::size_t m) const override
    {
        std::snprintf(a, n, "%s", config.node_name);
        std::snprintf(b, m, "%s", config.short_name);
    }
    bool switchMeshProtocol(chat::MeshProtocol p, bool) override
    {
        config.mesh_protocol = p;
        contacts.setActiveProtocol(p);
        return true;
    }
...
{
    static PreviewApp app;
    app::bindAppFacade(app);
}
