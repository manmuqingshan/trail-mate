...
static int page_for(const char* id) {
    ...
    for(int i=0;i<17;++i)if(std::strcmp(ids[i],id)==0)return i;
    return -1;
}
...
void preview_menu_enter()
{
    if(!menu_created) {
        ...
        flags.include_usb=true;flags.include_network=true;flags.include_walkie_talkie=true;flags.include_power_off=true;
        ...
        hooks.format_time=[](char* out,size_t n){std::snprintf(out,n,"09:41");return true;};
        hooks.show_main_menu=[](){native_open_page(10);};
...
void preview_menu_exit() {
    if(menu_created){ui::menu_runtime::setScene(ui::menu_runtime::Scene::App);lv_obj_add_flag(main_screen,LV_OBJ_FLAG_HIDDEN);}
}
...
void ui_switch_to_app(AppScreen* app,lv_obj_t* parent){active_app=app;const int page=page_for(app->stable_id());if(page>=0)native_open_page(page);else app->enter(parent);}
void ui_clear_active_app(){active_app=nullptr;}
void ui_exit_active_app(lv_obj_t* parent){if(active_app)active_app->exit(parent);active_app=nullptr;}
void menu_show(){native_open_page(10);}


void preview_power_enter() {
    preview_menu_enter();
    for(size_t i=0;i<ui::catalogCount(catalog);++i){auto* app=ui::catalogAt(catalog,i);if(std::strcmp(app->stable_id(),"shutdown")==0){app->enter(nullptr==main_screen?lv_screen_active():main_screen);return;}}
}
static lv_obj_t* cancel_button(lv_obj_t* obj) {
    ...
    for(uint32_t i=0;i<lv_obj_get_child_count(obj);++i)if(auto* found=cancel_button(lv_obj_get_child(obj,i)))return found;
    return nullptr;
}
void preview_power_exit(){if(auto* cancel=cancel_button(lv_screen_active()))lv_obj_send_event(cancel,LV_EVENT_CLICKED,nullptr);preview_menu_exit();}
