...
int get_int(const char* ns,const char* key,int fallback){auto i=values.find(key_for(ns,key));return i==values.end()?fallback:std::strtol(i->second.c_str(),nullptr,10);}
uint32_t get_uint(const char* ns,const char* key,uint32_t fallback){auto i=values.find(key_for(ns,key));return i==values.end()?fallback:std::strtoul(i->second.c_str(),nullptr,10);}
...
bool put_string(const char* ns,const char* key,const char* value){values[key_for(ns,key)]=value?value:"";return true;}
bool get_string(const char* ns,const char* key,std::string& out){auto i=values.find(key_for(ns,key));if(i==values.end())return false;out=i->second;return true;}
void remove_keys(const char* ns,const char* const* keys,std::size_t count){for(size_t i=0;i<count;++i)values.erase(key_for(ns,keys[i]));}
void clear_namespace(const char* ns){const auto prefix=std::string(ns)+"/";for(auto i=values.begin();i!=values.end();)if(i->first.compare(0,prefix.size(),prefix)==0)i=values.erase(i);else++i;}
...
bool scan(ScanResult* out,std::size_t capacity,std::size_t& count){count=capacity?1:0;if(count){std::snprintf(out[0].ssid,sizeof(out[0].ssid),"Preview network");out[0].rssi=-55;out[0].requires_password=false;}return true;}
}
