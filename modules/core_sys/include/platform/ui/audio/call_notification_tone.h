...
#include "platform/ui/audio/call_notification_adpcm.h"

#include <cstddef>
#include <cstdint>

namespace platform::ui::audio::call_notification
{

namespace asset = ::platform::ui::audio::assets;

constexpr uint32_t kSourceSampleRateHz = asset::kCallNotificationSampleRateHz;
...
static_assert(kPlaybackSampleRateHz % kSourceSampleRateHz == 0,
              "Call notification asset sample rate must divide the playback sample rate");
...
{
    int16_t predictor = asset::kCallNotificationInitialSample;
    int16_t current_sample = asset::kCallNotificationInitialSample;
...
{
    return state.sample_index < asset::kCallNotificationSampleCount ||
...
inline int16_t nextSourceSample(AdpcmPlaybackState& state)
{
    ...
    const uint8_t encoded = asset::kCallNotificationAdpcmData[nibble_index / 2U];
...
inline bool nextPlaybackSample(AdpcmPlaybackState& state, int16_t& sample)
{
    ...
    {
        if (state.sample_index >= asset::kCallNotificationSampleCount)
        {
            return false;
        }
