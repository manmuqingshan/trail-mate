    ...
    if (line < kMaxLines)
    {
        set_line(line++,
                 s_state.notice[0] != '\0'
                     ...
                     : "SELECT GPS OR 4G SETTINGS");
#else
                     : "SELECT TO TOGGLE GPS");
#endif
    }
...
{
    if (!::ui::tdeck_pro::text_shell::launch_app_by_stable_id("a7682e_phone"))
    {
...
void run_action(Action action)
{
    ...
    case Action::OpenCellularSettings:
#if defined(TRAIL_MATE_TDECK_PRO_A7682E)
        ...
        return;
#else
        set_notice("4G MODEM NOT PRESENT");
        break;
#endif
...
void create_root(lv_obj_t* parent, TextAppAdapter& adapter)
{
    ...
    case TextAppPageKind::SkyPlot:
        ...
        break;
    case TextAppPageKind::Settings:
        add_action("GPS ON/OFF", Action::ToggleGps, kMargin, kActionTop, 112);
#if defined(TRAIL_MATE_TDECK_PRO_A7682E)
        add_action("4G SETTINGS", Action::OpenCellularSettings, 126, kActionTop, 106);
#endif
        break;
    case TextAppPageKind::Network:
    ...
    set_text(s_state.footer, "W/S  ENT ACT  ESC BACK");
...
{
    // Returning to the text menu is the matching physical EPD lifecycle
    // boundary for the full refresh requested when this page was entered.
