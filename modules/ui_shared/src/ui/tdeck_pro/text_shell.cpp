...
void refresh_radio_summary()
{
    ...
    set_label_text_if_changed(s_state.title_label, device.active_protocol.c_str());
    ...
    std::snprintf(text,
                  ...
                  "%s %s",
                  device.region.c_str(),
                  device.modem_preset.c_str());
    set_label_text_if_changed(s_state.detail_label, text);
}
...
void show_app(AppScreen* app)
{
    ...
    // Crossing from the menu list into an application is a physical EPD
    // lifecycle boundary.  The board applies this to the next rendered frame.
...
void create_header()
{
    ...
    s_state.title_label = make_label(header, 112, LV_TEXT_ALIGN_LEFT);
    lv_obj_set_pos(s_state.title_label, 0, 0);
    set_label_text_if_changed(s_state.title_label, "PROTOCOL");
...
void create_footer()
{
    ...
    set_label_text_if_changed(s_state.detail_label, "RADIO --");
    ...
    lv_obj_set_height(s_state.command_label, kRowHeight);
    lv_obj_set_style_bg_color(s_state.command_label, lv_color_black(), 0);
    lv_obj_set_style_bg_opa(s_state.command_label, LV_OPA_COVER, 0);
    lv_obj_set_style_text_color(s_state.command_label, lv_color_white(), 0);
    lv_obj_set_style_pad_left(s_state.command_label, 2, 0);
    set_label_text_if_changed(s_state.command_label, "W/S MOVE ENT OPEN H HELP");

    s_state.help_panel = lv_obj_create(s_state.menu_panel);
    lv_obj_set_pos(s_state.help_panel, 16, 100);
    lv_obj_set_size(s_state.help_panel, 208, 120);
    lv_obj_set_style_bg_color(s_state.help_panel, lv_color_black(), 0);
    lv_obj_set_style_bg_opa(s_state.help_panel, LV_OPA_COVER, 0);
    lv_obj_set_style_border_width(s_state.help_panel, 1, 0);
    lv_obj_set_style_border_color(s_state.help_panel, lv_color_white(), 0);
    lv_obj_set_style_radius(s_state.help_panel, 0, 0);
    lv_obj_set_style_shadow_width(s_state.help_panel, 0, 0);
    lv_obj_set_style_pad_all(s_state.help_panel, 8, 0);
    lv_obj_clear_flag(s_state.help_panel, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_add_flag(s_state.help_panel, LV_OBJ_FLAG_HIDDEN);

    s_state.help_label = make_label(s_state.help_panel, 190, LV_TEXT_ALIGN_LEFT);
    lv_obj_set_style_text_color(s_state.help_label, lv_color_white(), 0);
    set_label_text_if_changed(s_state.help_label,
                              "KEYBOARD HELP\n\nW/S  MOVE\nENTER  OPEN\nH  CLOSE HELP");
}
...
void toggle_help()
{
    if (!valid(s_state.help_panel))
    {
        return;
    }

    if (lv_obj_has_flag(s_state.help_panel, LV_OBJ_FLAG_HIDDEN))
    {
        lv_obj_clear_flag(s_state.help_panel, LV_OBJ_FLAG_HIDDEN);
        lv_obj_move_foreground(s_state.help_panel);
    }
    else
    {
        lv_obj_add_flag(s_state.help_panel, LV_OBJ_FLAG_HIDDEN);
...
void set_walkie_recording(bool recording)
{
    ...
    set_label_text_if_changed(
        ...
        recording ? "PTT TRANSMIT  RELEASE STOP" : "W/S MOVE ENT OPEN H HELP");
}
