                         ...
                         uint8_t coding_rate)
{
    ...
    {
        std::snprintf(region, sizeof(region), "%.3fMHz",
...
bool RuntimeDeviceStatusSource::buildDeviceStatusSnapshot(
    ui::device::DeviceStatusSnapshot& out) const
{
    ...
    else if (protocol == chat::MeshProtocol::MeshCore)
    {
        ...
        if (mesh.meshcore_region_preset > 0)
        {
            if (const chat::meshcore::RegionPreset* preset =
                    chat::meshcore::findRegionPresetById(
...
{
    static RuntimeDeviceStatusSource source;
    return source;
}
