...
def convert_png_to_lvgl(input_path, output_path, image_name="img"):
    ...
    # LVGL v9 RGB565A8 stores a full RGB565 plane followed by a full A8 plane.
    # The image stride is the color plane stride, not RGB565+alpha per row.
    stride = width * 2
    ...
    # Convert each pixel into planar RGB565A8 format.
    for y in range(height):
        for x in range(width):
            ...
            # Pack color plane as little-endian RGB565.
            color_data.append(rgb565_val & 0xFF)  # Low byte
            color_data.append((rgb565_val >> 8) & 0xFF)  # High byte
            alpha_data.append(a)
    ...
    output_lines.append(f"#ifndef LV_ATTRIBUTE_IMAGE_{attr_name}")
    output_lines.append(f"#define LV_ATTRIBUTE_IMAGE_{attr_name}")
    output_lines.append("#endif")
    output_lines.append("")
    output_lines.append(
