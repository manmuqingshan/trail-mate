...
- `RuntimeMapWorkspaceSource` and `RuntimeMapActionSink` now bind GPS/map/team
  runtime state behind read/write adapters.
...
- `UConsoleMapWorkspaceModel` now exposes a portable
  `presentation_workspace` and consumes `RuntimeGpsStatusSource` instead of
