...
| RCNF-003 | Address projection | Trail Mate still carries 32-bit node ids as compatibility projections, while V9 node-store persistence, chat message metadata, core in-memory conversation keys, and incoming duplicate detection keep/use full Reticulum destination identity when available | intentional-subset | Continue migrating persistent chat-store grouping, unread keys, SD/index paths, and UI presentation conversation ids to carry Reticulum destination identity |
