...
* The `vmp_tx` and `vmp_play` FreeRTOS stacks are a separate, deliberate internal-SRAM exception: each is **10 KiB** and exists only while recording/sending or playing. ESP-IDF's task-depth argument is bytes. Codec2 plus the Pager I2S/codec/I2C call chain cannot safely run in the former 4 KiB/3 KiB tasks, but 16 KiB could not be allocated once Wi-Fi fragmented the Pager's internal heap. These stacks MUST NOT be moved to PSRAM because the Arduino Codec2/I2S path may reach ROM/cache-disabled code; they are released by `vTaskDelete`, and `[VMP][Mem]` records their high-water mark after capture/playback and at task completion. A failed create records internal and PSRAM free/largest-block diagnostics.
