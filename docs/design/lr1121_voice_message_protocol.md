...
* The `vmp_tx` and `vmp_play` FreeRTOS stacks are a separate, deliberate internal-SRAM exception: each is **8 KiB** and exists only while recording/sending or playing. ESP-IDF's task-depth argument is bytes. Codec2's formerly automatic 4–10 KiB FFT/model workspaces now live in a short-lived, PSRAM-only Codec2 instance; this prevents those non-DMA buffers from consuming the audio task stack or Pager internal heap. The remaining 8 KiB stays internal because the Arduino I2S/codec path may reach ROM/cache-disabled code. It is released by `vTaskDelete`; `[VMP][Mem]` records its high-water mark after capture/playback and at task completion, while a failed create records internal and PSRAM free/largest-block diagnostics. Pager refuses Codec2 creation if PSRAM cannot satisfy those workspaces; it never silently falls back to internal SRAM.
