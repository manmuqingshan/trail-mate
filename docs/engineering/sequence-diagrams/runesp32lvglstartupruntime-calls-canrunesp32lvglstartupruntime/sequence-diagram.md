# Dynamic collaboration: runEsp32LvglStartupRuntime calls canRunEsp32LvglStartupRuntime
...
runEsp32LvglStartupRuntime in apps/esp32_lvgl/src/esp32_lvgl_startup_runtime.cpp calls canRunEsp32LvglStartupRuntime in apps/esp32_lvgl/src/esp32_lvgl_startup_runtime.cpp.

## How to read the picture

- This Sequence Diagram focuses on the technical collaboration fragment of runEsp32LvglStartupRuntime calling canRunEsp32LvglStartupRuntime.
...
- Description: runEsp32LvglStartupRuntime is the source participant of the current Sequence Diagram, used to interpret runEsp32LvglStartupRuntime calls in apps/esp32_lvgl/src/esp32_lvgl_startup_runtime.cpp canRunEsp32LvglStartupRuntime in apps/esp32_lvgl/src/esp32_lvgl_startup_runtime.cpp.
...
- Relationship meaning: runEsp32LvglStartupRuntime -> canRunEsp32LvglStartupRuntime indicates the dependency direction of the current fragment; if the relationship is only import, it can only indicate static dependencies and cannot directly prove the runtime order.
...
- Element type: component
- Description: canRunEsp32LvglStartupRuntime is the target participant of the current Sequence Diagram and is used to interpret runEsp32LvglStartupRuntime calls in apps/esp32_lvgl/src/esp32_lvgl_startup_runtime.cpp canRunEsp32LvglStartupRuntime in apps/esp32_lvgl/src/esp32_lvgl_startup_runtime.cpp.
- Technical role: Message receiver/relying party: It provides the ability to be referenced or called by the current fragment.
- Why it appears: Local repository evidence calls were observed between apps/esp32_lvgl/src/esp32_lvgl_startup_runtime.cpp and apps/esp32_lvgl/src/esp32_lvgl_startup_runtime.cpp, so canRunEsp32LvglStartupRuntime was put into sequence.
- Relationship meaning: runEsp32LvglStartupRuntime -> canRunEsp32LvglStartupRuntime indicates that the target capability is dependent on the current fragment; it is necessary to combine the call evidence to confirm whether it is a runtime call, a type reference or a static import.
...
- Generated from local repository evidence Dynamic collaboration: runEsp32LvglStartupRuntime calls canRunEsp32LvglStartupRuntime.
