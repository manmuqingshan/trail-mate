# Dynamic collaboration: setBootLog calls unlockUi
...
setBootLog in apps/esp32_lvgl/src/esp32_lvgl_startup_runtime.cpp calls unlockUi in apps/esp32_lvgl/src/esp32_lvgl_startup_runtime.cpp.

## How to read the picture

- This Sequence Diagram focuses on the technical collaboration fragment of setBootLog calling unlockUi.
...
```mermaid
sequenceDiagram
  participant Source as setBootLog
...
- Description: setBootLog is the source participant of the current Sequence Diagram and is used to interpret setBootLog calls unlockUi in apps/esp32_lvgl/src/esp32_lvgl_startup_runtime.cpp.
...
- Relationship meaning: setBootLog -> unlockUi indicates the dependency direction of the current fragment; if the relationship is only import, it can only indicate static dependencies and cannot directly prove the runtime order.
...
- Element type: component
 - Description: unlockUi is the target actor of the current Sequence Diagram, used to interpret setBootLog calls unlockUi in apps/esp32_lvgl/src/esp32_lvgl_startup_runtime.cpp.
...
- Relationship meaning: setBootLog -> unlockUi means that the target capability is dependent on the current fragment; it needs to be combined with the call evidence to confirm whether it is a runtime call, a type reference or a static import.
...
- Generate dynamic collaboration from local warehouse evidence: setBootLog calls unlockUi.
