...
function 位于 firmware/c6_companion/tests/test_tm_services_functional.cpp#L97，协调多个外部对象或能力，用于解释技术协作和变更影响面。

## 图的读法

- 这张 Component Diagram 关注 main 这个 function，展示它所在文件、所属模块以及复用迹象、外部协作迹象。
- 复用迹象用于判断它是否是共享核心或公共接口；外部协作迹象用于判断它是否承担编排、聚合或桥接职责。
- 代码锚点是 firmware/c6_companion/tests/test_tm_services_functional.cpp#L97。
...
- 组件类型：function
- 所属模块：firmware
- 代码锚点：firmware/c6_companion/tests/test_tm_services_functional.cpp#L97
...
- 元素类型：package
- 说明：firmware 是 服务对象：main 所属的 package/module 边界，用来判断该组件是局部实现细节还是跨模块协作点。
- 技术角色：组件归属边界：它定义当前组件默认应该服务的工程上下文。
- 为什么出现：组件不能脱离 package 解释；同一个符号如果位于不同 package，可能代表完全不同的职责、所有权和变更影响面。
- 关系意义：firmware -> 服务对象：main 表示该组件由这个技术边界承载；它的角色必须由入口、调用、导出、测试或配置证据来解释，而不是只由目录位置解释。
- 下钻意图：下钻 package 可以查看 firmware 的跨模块依赖、结构协作和热点，从边界层解释该组件为何出现在这里。
- 业务关联：如果 服务对象：main 被业务 Use Case 调用，那么 firmware 是该业务能力的候选技术落点。
...
- 问题：
  - 当前仓库证据尚未证明 服务对象：main 属于 firmware 的稳定职责，暂按候选归属处理。
- 下钻：[模块边界：firmware](../../package-diagrams/firmware/package-diagram.md) - 回到 firmware 的包级边界，确认 服务对象：main 是否属于这个模块的稳定职责。

### firmware/c6_companion/tests/test_tm_services_functional.cpp

- 元素类型：file
- 说明：firmware/c6_companion/tests/test_tm_services_functional.cpp 是当前组件的证据文件，说明 服务对象：main 的技术职责可以追溯到具体代码位置。
...
- 变更影响：修改 firmware/c6_companion/tests/test_tm_services_functional.cpp 可能影响该组件图、相关 sequence、热点判断，以及引用该组件的业务下钻文档。
- 置信度：high
- 证据：
  - firmware/c6_companion/tests/test_tm_services_functional.cpp
  - firmware/c6_companion/tests/test_tm_services_functional.cpp#L97
...
- 元素类型：component
- 说明：服务对象：main 是当前 Component Diagram 的中心技术对象；这张图用它来解释职责、协作压力和下钻路径。
- 技术角色：关键技术对象：它需要结合复用迹象、外部协作迹象、文件位置和下钻图判断具体职责。
- 为什么出现：本地仓库证据将它识别为关键组件或符号，且它具有可定位文件 firmware/c6_companion/tests/test_tm_services_functional.cpp、被引用/调用关系和对外依赖/调用关系。
...
- 业务关联：服务对象：main 不是业务用例，但可能是业务能力落地时经过的技术节点。 从路径上无法直接判断业务角色，需要结合组织/过程模型的 Use Case 证据确认。 如果组织/过程模型的 Use Case 下钻图引用它，应在业务文档中说明它承担入口、编排、领域规则、适配器还是基础设施职责。
- 变更影响：修改 服务对象：main 可能影响 firmware/c6_companion/tests/test_tm_services_functional.cpp 中的入口逻辑、调用关系和引用它的业务/工程文档。
- 置信度：high
- 证据：
  - firmware/c6_companion/tests/test_tm_services_functional.cpp
  - 组件类型：function
