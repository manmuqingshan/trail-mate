                                         ...
                                         bool want_response)
{
    ...
    const bool effective_want_response = send_policy.effective_want_response;
    ...
    const ::chat::NodeId wire_dest = send_policy.wire_dest;
    size_t key_len = 0;
    const uint8_t* key = selectKey(config_, out_channel, &key_len);
...
void MeshtasticRadioAdapter::setMqttProxySettings(const MqttProxySettings& settings)
{
    ...
    if (changed)
    {
        logMeshtasticRx("[nrf52][mt][mqtt] settings enabled=%u proxy=%u enc=%u root=%s "
...
bool MeshtasticRadioAdapter::hasMqttProxyMessage() const
{
...
bool MeshtasticRadioAdapter::handleMqttProxyMessage(const meshtastic_MqttClientProxyMessage& msg)
{
    ...
    {
        logMeshtasticRx("[nrf52][mt][mqtt] proxy reject reason=%s variant=%u len=%u enabled=%u proxy=%u\n",
                        ::chat::meshtastic::mqttProxyRejectReasonName(inbound_reason),
                        static_cast<unsigned>(msg.which_payload_variant),
    ...
    {
        logMeshtasticRx("[nrf52][mt][mqtt] proxy reject reason=%s topic='%s' len=%u\n",
...
void MeshtasticRadioAdapter::handleRawPacket(const uint8_t* data, size_t size)
{
    ...
    if (header.channel == 0)
    {
        (void)getNodeLastChannel(header.from, &channel);
        ...
        {
            (void)primary_hash;
            (void)executePkiResync(::chat::runtime::MeshtasticPkiResyncCause::LocalPkiNotReady,
                                   header.from,
                                   header.id,
                                   ::chat::ChannelId::PRIMARY);
            return;
    ...
    if (!decoded_ok)
    {
        ...
        {
            size_t probe_plain_len = rx.plain.size();
            if (resolvePacketWithKnownKeys(config_,
                                           header,
                                           payload,
                                           payload_size,
    ...
    {
        rememberNodeLastChannel(header.from, channel, millis());
    }
    ...
    if (decoded_ok)
    {
        ...
        {
            logMeshtasticRx("[gat562][mt] runtime payload drop port=%u len=%u cap=%u\n",
                            static_cast<unsigned>(decoded.portnum),
                            static_cast<unsigned>(decoded.payload.size),
    ...
    if (decoded_ok && decoded.portnum == meshtastic_PortNum_NODEINFO_APP)
    {
        ...
        if (reply_policy.should_reply)
        {
            ...
            {
                setNodeInfoReplyMs(header.from, now_ms);
            }
        }
    }
...
void MeshtasticRadioAdapter::maybeBroadcastNodeInfoAfterPeerAnnouncement(::chat::NodeId from_node,
                                                                         uint32_t now_ms,
                                                                         ::chat::ChannelId channel,
                                                                         bool from_mqtt)
{
    ...
    if (!policy.should_announce)
    {
        ...
        {
            logMeshtasticRx("[gat562][mt] nodeinfo announce suppressed from=%08lX age=%lu\n",
                            static_cast<unsigned long>(from_node),
                            static_cast<unsigned long>(policy.age_ms));
        }
        return;
    }

    if (buildAndQueueNodeInfo(kBroadcastNode, false, channel))
    {
        logMeshtasticRx("[gat562][mt] nodeinfo announce after peer from=%08lX ch=%u\n",
                        static_cast<unsigned long>(from_node),
                        static_cast<unsigned>(channel));
    }
    else
    {
        logMeshtasticRx("[gat562][mt] nodeinfo announce fail after peer from=%08lX ch=%u\n",
                        static_cast<unsigned long>(from_node),
                        static_cast<unsigned>(channel));
    }
}
...
bool MeshtasticRadioAdapter::executeProtocolEffect(const ::chat::runtime::ProtocolEffect& effect)
{
    bool ok = false;
    ::chat::runtime::visitProtocolEffect(
        ...
        [this, &ok](const auto& item)
        {
            ...
            {
                ok = buildAndQueueNodeInfo(item.peer, item.want_response, item.channel);
            }
            else if constexpr (std::is_same_v<Effect, ::chat::runtime::SendRoutingErrorEffect>)
            {
                size_t key_len = 0;
                const uint8_t* key = selectKey(config_, item.channel, &key_len);
                const uint8_t channel_hash = channelHashFor(config_, item.channel);
                ok = sendRoutingError(item.peer,
                                      item.request_id,
                                      channel_hash,
                                      item.channel,
                                      key,
            ...
            else if constexpr (std::is_same_v<Effect, ::chat::runtime::ForgetPeerKeyEffect>)
            {
...
bool MeshtasticRadioAdapter::executePkiResync(::chat::runtime::MeshtasticPkiResyncCause cause,
                                              ::chat::NodeId peer,
...
{
    return packet_history_.find(sender, packet_id);
}
                                                                                  ...
                                                                                  bool allow_update)
{
    ...
    const uint32_t now_ms = millis();
    packet_history_.pruneExpired(now_ms, kPacketHistoryTimeoutMs);
    ...
    if (!existing)
    {
        existing = packet_history_.allocateDropOldest();
...
void MeshtasticRadioAdapter::rememberLocalPacket(const ::chat::meshtastic::PacketHeaderWire& header)
{
    ...
    if (!entry)
    {
        entry = packet_history_.allocateDropFirst();
                                                 ...
                                                 const ::chat::RxMeta& rx_meta)
{
    ...
    if (decoded.request_id != 0 && header.to == node_id_)
    {
        ...
        {
            const auto cause = (reason == meshtastic_Routing_Error_PKI_UNKNOWN_PUBKEY)
                                   ? ::chat::runtime::MeshtasticPkiResyncCause::PeerReportsUnknownPubkey
                                   : ::chat::runtime::MeshtasticPkiResyncCause::PeerReportsNoChannel;
            (void)executePkiResync(cause, header.from, 0, channel);
        }
        return;
    }
...
MeshtasticRadioAdapter::PkiNodeKeyEntry*
MeshtasticRadioAdapter::findPkiNodeKey(::chat::NodeId node_id)
{
...
const MeshtasticRadioAdapter::PkiNodeKeyEntry*
MeshtasticRadioAdapter::findPkiNodeKey(::chat::NodeId node_id) const
{
...
MeshtasticRadioAdapter::PkiNodeKeyEntry*
MeshtasticRadioAdapter::upsertPkiNodeKey(::chat::NodeId node_id,
                                         const uint8_t* key,
                                         ...
                                         bool* out_evicted)
{
    ...
    const uint32_t seen = last_seen_s != 0 ? last_seen_s : nowSeconds();
    if (auto* existing = findPkiNodeKey(node_id))
    {
        const bool changed = std::memcmp(existing->key.data(), key, existing->key.size()) != 0;
        if (changed)
        {
            std::memcpy(existing->key.data(), key, existing->key.size());
        }
    ...
    *slot = PkiNodeKeyEntry{};
    ...
    std::memcpy(slot->key.data(), key, slot->key.size());
    if (out_changed)
    {
        *out_changed = true;
    }
...
void MeshtasticRadioAdapter::clearPkiNodeKeys()
{
    for (auto& entry : pki_node_keys_)
    {
...
bool MeshtasticRadioAdapter::erasePkiNodeKey(::chat::NodeId node_id)
{
...
std::size_t MeshtasticRadioAdapter::pkiNodeKeyCount() const
{
    std::size_t count = 0;
    for (const auto& entry : pki_node_keys_)
    {
...
MeshtasticRadioAdapter::NodeRuntimeEntry*
MeshtasticRadioAdapter::findNodeRuntime(::chat::NodeId node_id)
{
...
const MeshtasticRadioAdapter::NodeRuntimeEntry*
MeshtasticRadioAdapter::findNodeRuntime(::chat::NodeId node_id) const
{
...
MeshtasticRadioAdapter::NodeRuntimeEntry*
MeshtasticRadioAdapter::upsertNodeRuntime(::chat::NodeId node_id, uint32_t now_ms)
{
    if (node_id == 0)
    {
        return nullptr;
...
void MeshtasticRadioAdapter::eraseNodeRuntime(::chat::NodeId node_id)
{
...
bool MeshtasticRadioAdapter::getNodeLastChannel(::chat::NodeId node_id,
                                                ::chat::ChannelId* out) const
{
...
void MeshtasticRadioAdapter::rememberNodeLastChannel(::chat::NodeId node_id,
                                                     ::chat::ChannelId channel,
                                                     uint32_t now_ms)
{
...
uint32_t MeshtasticRadioAdapter::getNodeInfoReplyMs(::chat::NodeId node_id) const
{
    const auto* entry = findNodeRuntime(node_id);
    return entry ? entry->nodeinfo_reply_ms : 0;
}

void MeshtasticRadioAdapter::setNodeInfoReplyMs(::chat::NodeId node_id, uint32_t now_ms)
{
    if (auto* entry = upsertNodeRuntime(node_id, now_ms))
    {
...
void MeshtasticRadioAdapter::loadPkiNodeKeys()
{
    ...
    for (size_t index = 0; index < key_count; ++index)
    {
        ...
        if (evicted)
        {
            logMeshtasticRx("[gat562][mt] pki key load evicted oldest node=%08lX cap=%u\n",
                                            ...
                                            size_t key_len)
{
    ...
    bool changed = false;
    bool evicted = false;
    if (!upsertPkiNodeKey(node_id, key, nowSeconds(), &changed, &evicted))
    {
        return;
    }
    ...
    if (changed || evicted)
    {
        ...
        logMeshtasticRx("[gat562][mt] pki key updated node=%08lX evicted=%u depth=%u\n",
                        static_cast<unsigned long>(node_id),
                        evicted ? 1U : 0U,
                        static_cast<unsigned>(pkiNodeKeyCount()));
    }

    // Refresh recency even when the key bytes did not change.
...
void MeshtasticRadioAdapter::forgetNodePublicKey(::chat::NodeId node_id)
{
    if (node_id == 0)
    {
        return;
...
bool MeshtasticRadioAdapter::savePkiKeysToPrefs()
{
    ...
    for (const auto& item : pki_node_keys_)
    {
        ...
        std::memcpy(entry.public_key, item.key.data(), sizeof(entry.public_key));
                                               ...
                                               uint8_t* out_plain, size_t* out_plain_len)
{
    ...
    std::memcpy(shared, key_entry->key.data(), sizeof(shared));
    std::memcpy(local_priv, pki_private_key_.data(), sizeof(local_priv));
    if (!Curve25519::dh2(shared, local_priv))
    {
        (void)executePkiResync(::chat::runtime::MeshtasticPkiResyncCause::PeerKeyStale,
                               from,
                               packet_id,
                               ::chat::ChannelId::PRIMARY);
        return false;
                                                     ...
                                                     ::chat::ChannelId* out_channel) const
{
    ...
    ::chat::ChannelId channel = match.channel;
    size_t key_len = 0;
    const uint8_t* key = nullptr;
    uint8_t hash = 0;
    ...
    else if (match.known && match.channel == ::chat::ChannelId::PRIMARY)
    {
    ...
    else if (match.known && match.channel == ::chat::ChannelId::SECONDARY)
    {
                                                ...
                                                const char* gateway_id)
{
    ...
    {
        logMeshtasticRx("[nrf52][mt][mqtt] downlink reject reason=%s enabled=%u proxy=%u\n",
                        ::chat::meshtastic::mqttProxyRejectReasonName(
    ...
    {
        logMeshtasticRx("[nrf52][mt][mqtt] downlink reject reason=%s channel='%s' primary='%s' p_down=%u secondary='%s' s_down=%u\n",
    ...
    {
        logMeshtasticRx("[nrf52][mt][mqtt] downlink ignore reason=%s gateway='%s' from=%08lX id=%08lX\n",
    ...
    if (packet.which_payload_variant == meshtastic_MeshPacket_encrypted_tag)
    {
        ...
        {
            logMeshtasticRx("[nrf52][mt][mqtt] downlink reject reason=%s encrypted=%u wire_cap=%u\n",
                            ::chat::meshtastic::mqttProxyRejectReasonName(
                                ::chat::meshtastic::MqttProxyRejectReason::PayloadTooLarge),
                            static_cast<unsigned>(enc_size),
                            static_cast<unsigned>(scratch.wire.size()));
            return false;
        }
    ...
    else
    {
        ...
        {
            logMeshtasticRx("[nrf52][mt][mqtt] downlink reject reason=%s port=%u\n",
                            ::chat::meshtastic::mqttProxyRejectReasonName(
        ...
        {
            logMeshtasticRx("[nrf52][mt][mqtt] downlink reject reason=%s id=%08lX ch=0x%02X data=%u\n",
                            ::chat::meshtastic::mqttProxyRejectReasonName(
                                ::chat::meshtastic::MqttProxyRejectReason::WireBuildFailed),
                            static_cast<unsigned long>(packet.id),
                            static_cast<unsigned>(channel_hash),
                            static_cast<unsigned>(dstream.bytes_written));
            return false;
        }
    ...
    logMeshtasticRx("[nrf52][mt][mqtt] downlink inject topic_ch='%s' gateway='%s' from=%08lX to=%08lX id=%08lX\n",
                    channel_id ? channel_id : "",
                    gateway_id ? gateway_id : "",
                                                           ...
                                                           ::chat::ChannelId channel_index)
{
    ...
    if (publish_reason != ::chat::meshtastic::MqttProxyRejectReason::None)
    {
        ...
        {
            logMeshtasticRx("[nrf52][mt][mqtt] uplink reject reason=%s from=%08lX id=%08lX ch=%u "
                            "idx=%u enabled=%u proxy=%u up=%u/%u\n",
