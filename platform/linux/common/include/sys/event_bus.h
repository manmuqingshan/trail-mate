 ...
 * normal text notifications retain their event footprint. Attachment media is
 * deliberately not copied into this transport event.
 */
 ...
 *
 * The voice runtime owns the media and metadata; this short-lived event
 * carries only the attachment identifier used for diagnostics and routing.
 * It shares EventType::ChatNewMessage so the chat UI refreshes a voice item
 * through the same incoming-message flow as text.
 */
...
struct NodeInfoUpdateEvent : public Event
{
    ...
    NodeInfoUpdateEvent(uint32_t id, const char* sname, const char* lname, float s, float rssi_val,
        ...
        : Event(EventType::NodeInfoUpdate),
          ...
          has_public_key_state(has_pubkey_state),
